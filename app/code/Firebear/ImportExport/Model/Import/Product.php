...
class Product extends MagentoProduct
{
        ...
        if (!empty($this->_parameters['configurable_switch']) && isset($rowData['product_type'])
            ...
            if ($skuConf) {
                ...
                if (!empty($arrayConf)) {
                    ...
                    $arrayConf[ProductInterface::VISIBILITY] =
                        $attrs[ProductInterface::VISIBILITY] ?? Visibility::VISIBILITY_NOT_VISIBLE;
                    $arrayConf[ProductInterface::STATUS] = $attrs[ProductInterface::STATUS] ?? Status::STATUS_ENABLED;
                    ...
                    $arrayConf[Product\ConfigurationVariations::FIELD_CONF_IMPORT]['category_ids']
                        = $this->configVariations->processRowCategories($rowData, $this->getMultipleValueSeparator());
                    $configurableData[(string) $skuConf][] = $arrayConf;
                }
            }
        } elseif (isset($rowData['product_type']) && $rowData['product_type'] == TypeConfigurable::TYPE_CODE) {
            if (!empty($configurableData[$rowData['sku']])) {
                $configurableData[$rowData['sku']]['configurable_product_data'][ProductInterface::STATUS] =
                    $attrs[ProductInterface::STATUS] ?? Status::STATUS_ENABLED;
                $configurableData[$rowData['sku']]['configurable_product_data'][ProductInterface::VISIBILITY] =
    ...
    protected function saveConfigurationVariations($data, $existingImages = [])
    {
        if (!empty($data)) {
            ...
            foreach ($data as $skuConf => $elements) {
                ...
                if (!empty($elements['configurable_product_data'])) {
                    $configurableProductStatus =
                        $elements['configurable_product_data'][ProductInterface::STATUS];
                    $configurableProductVisibility =
                        $elements['configurable_product_data'][ProductInterface::VISIBILITY];
                    unset($elements['configurable_product_data']);
                }
                ...
                try {
                    ...
                    if ($this->_parameters['configurable_create'] && !$this->isSkuExist($checkSku)) {
                        try {
                            $child = current($data[$skuConf]);
                            $updateData[self::COL_SKU] = $skuConf;
                            $updateData['eav_attributes'] = $child[Product\ConfigurationVariations::FIELD_COPY_VALUE];
                            ...
                            $updateData['attribute_set_id']
                                = $child[Product\ConfigurationVariations::FIELD_CONF_IMPORT]['attribute_set_id'];
                            $updateData['type_id'] = TypeConfigurable::TYPE_CODE;
                            $updateData['website_ids']
                                = $child[Product\ConfigurationVariations::FIELD_CONF_IMPORT]['website_ids'];
                            $updateData['category_ids']
                                = $child[Product\ConfigurationVariations::FIELD_CONF_IMPORT]['category_ids'];
                        ...
                        if ($this->isSkuExist($checkSku)) {
                            ...
                            $productChild = current($data[$skuConf]);
                            $updateData['eav_attributes']
                                = $productChild[Product\ConfigurationVariations::FIELD_COPY_VALUE];
                            $updateData['eav_attributes'][ProductInterface::STATUS] =
                                $configurableProductStatus ?? Status::STATUS_ENABLED;
                            $updateData['eav_attributes'][ProductInterface::VISIBILITY] =
                                $configurableProductVisibility ?? Visibility::VISIBILITY_BOTH;
                            ...
                            $updateData['attribute_set_id']
                                = $productChild[Product\ConfigurationVariations::FIELD_CONF_IMPORT]['attribute_set_id'];
                            $updateData['type_id'] = TypeConfigurable::TYPE_CODE;
                            $updateData['website_ids']
                                = $productChild[Product\ConfigurationVariations::FIELD_CONF_IMPORT]['website_ids'];
                            $updateData['category_ids']
                                = $productChild[Product\ConfigurationVariations::FIELD_CONF_IMPORT]['category_ids'];
    ...
    public function validateRow(array $rowData, $rowNum)
    {
        ...
        if (empty($this->_parameters['generate_url']) &&
            ...
            if (isset($urlKeys[$urlKey]) && $urlKeys[$urlKey] != $sku) {
                $message = sprintf(
        ...
        while ($source->valid() || $bunchRows) {
            ...
            if ($source->valid()) {
                ...
                if ($this->validateRow($rowData, $source->key())) {
                    ...
                    if (isset($rowData[self::COL_TYPE]) && !$this->validateRowByProductType($rowData, $source->key())) {
    ...
    {
        if ($this->isSkuExist($rowData[self::COL_SKU]) && !isset($rowData[self::URL_KEY])) {
